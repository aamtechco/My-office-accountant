...
export default function VatOffices(){
  const [list, setList] = useState([]); const [name,setName]=useState(''); const [addr,setAddr]=useState('')
  useEffect(()=> fetch(),[])
  async function fetch(){ const { data } = await supabase.from('vat_offices').select('*').order('name'); setList(data||[]) }
  async function add(){ if(!name) return alert('name'); await supabase.from('vat_offices').insert([{name,address:addr}]); setName(''); setAddr(''); fetch() }
  async function edit(id){ const n = prompt('New name'); if(!n) return; await supabase.from('vat_offices').update({name:n}).eq('id',id); fetch() }
  return (
    <div>
